...
resource "aws_ecr_repository" "api_gateway" {
  ...
  tags = merge(local.common_tags, {
    Name = "${local.name_prefix}-api-gateway-ecr"
  })
}

# ECR Repository for Workflow Agent
resource "aws_ecr_repository" "workflow_agent" {
  name                 = "agent-team/workflow-agent"
  image_tag_mutability = "MUTABLE"
  ...
  tags = merge(local.common_tags, {
    Name = "${local.name_prefix}-workflow-agent-ecr"
  })
}

# ECR Repository for Workflow Engine
resource "aws_ecr_repository" "workflow_engine" {
  name                 = "agent-team/workflow-engine"
  image_tag_mutability = "MUTABLE"
  ...
  tags = merge(local.common_tags, {
    Name = "${local.name_prefix}-workflow-engine-ecr"
  })
}
...
# ECR Lifecycle Policy for Workflow Agent
resource "aws_ecr_lifecycle_policy" "workflow_agent" {
  repository = aws_ecr_repository.workflow_agent.name
