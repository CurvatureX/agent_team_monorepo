...
class AIProviderInterface(ABC):
    ...
    def create_error_response(
        self, error_type: ErrorType, error_message: str, raw_error: Any = None
    ) -> AIResponse:
    ...
    def create_success_response(
        self, content: str, raw_response: Any = None, metadata: Optional[Dict[str, Any]] = None
    ) -> AIResponse:
        """Create a standardized success response."""
        # Final validation of content
        is_error, error_msg = self.detect_error_in_content(content)
        if is_error:
            return self.create_error_response(ErrorType.RESPONSE_ERROR, error_msg, raw_response)

        return AIResponse(
            success=True,
            content=content,
            raw_response=raw_response,
            metadata={"provider": self.__class__.__name__, **(metadata or {})},
        )
