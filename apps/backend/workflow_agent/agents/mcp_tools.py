...
class GetNodeDetailsInput(BaseModel):
    ...
    nodes: List[NodeQuery] = Field(..., description="List of nodes to get details for")
    include_examples: bool = Field(True, description="Include usage examples")
    include_schemas: bool = Field(True, description="Include input/output schemas")
...
class MCPToolCaller:
    ...
    async def get_node_types(self, type_filter: Optional[str] = None) -> Dict[str, Any]:
        ...
        """
        try:
            payload = {"name": "get_node_types", "tool_name": "get_node_types", "arguments": {}}
    ...
    async def get_node_details(
        ...
        try:
            ...
            node_list = [
                f"{n.get('node_type', 'unknown')}:{n.get('subtype', 'unknown')}" for n in nodes
            ]
            logger.info(
                f"📦 Getting details for {len(nodes)} nodes: {', '.join(node_list[:3])}{'...' if len(node_list) > 3 else ''}"
            )
    ...
    async def call_tool(self, tool_name: str, tool_args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        args_summary = (
            f"nodes={len(tool_args.get('nodes', []))} items"
            if "nodes" in tool_args
            else str(tool_args)
        )
            ...
            if result:
                if isinstance(result, dict):
                    if "nodes" in result:
                        ...
                        logger.info(
                            f"✅ MCP Response: {len(result.get('node_types', []))} node types"
                        )
                    else:
    ...
    def get_langchain_tools(self) -> List[Tool]:
        ...
        async def _get_node_details(
            nodes: List[Dict[str, str]], include_examples: bool = True, include_schemas: bool = True
        ) -> str:
            result = await self.get_node_details(nodes, include_examples, include_schemas)
            return json.dumps(result, indent=2)
