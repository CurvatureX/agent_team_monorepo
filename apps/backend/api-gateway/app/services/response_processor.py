...
class UnifiedResponseProcessor:
    ...
    def _process_workflow_generation(agent_state: Dict[str, Any]) -> Dict[str, Any]:
        """处理工作流生成阶段响应"""
        conversations = agent_state.get("conversations", [])
        current_workflow = agent_state.get("current_workflow_json") or agent_state.get(
            "current_workflow"
        )
        ...
        if current_workflow:
            ...
            return {
                ...
                "content": {"text": latest_message, "stage": "workflow_generation"},
            }
        else:
            return {
                "type": "ai_message",
                "content": {"text": latest_message or "正在生成工作流...", "stage": "workflow_generation"},
            }
    ...
    def _process_debug(agent_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if current_workflow:
            ...
            return {
                "type": "workflow",
                "workflow": workflow_data,
                "content": {"text": latest_message, "stage": "debug"},
            }
        else:
            return {
                "type": "ai_message",
    ...
    def _process_completed(agent_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if current_workflow:
            ...
            return {
                "type": "workflow",
                "workflow": workflow_data,
                "content": {"text": latest_message or "工作流已完成生成", "stage": "completed"},
            }
        else:
            # 如果没有工作流数据，可能是错误情况
            return {
    ...
    def create_error_response(
        error_message: str, error_code: str = "UNKNOWN_ERROR", stage: str = "unknown"
    ) -> Dict[str, Any]:
        """创建错误响应"""
        return {
            "type": "error",
    ...
    def create_status_response(
        message: str, stage: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """创建状态响应"""
        return {
            "type": "status",
            "content": {"message": message, "stage": stage, "metadata": metadata or {}},
        }
