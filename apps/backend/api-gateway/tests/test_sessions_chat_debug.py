...
class TestSessionsChatDebug:
    ...
    def test_create_session(self):
        ...
        try:
            response = httpx.post(
                f"{self.base_url}/api/v1/app/sessions", json=session_data, headers=headers
            )
    ...
    def test_get_session(self):
        ...
        try:
            response = httpx.get(
                f"{self.base_url}/api/v1/app/sessions/{self.session_id}", headers=headers
            )
    ...
    def test_list_sessions(self):
        ...
        try:
            response = httpx.get(f"{self.base_url}/api/v1/app/sessions", headers=headers)
            self.log(f"📥 List sessions response status: {response.status_code}")

            if response.status_code == 200:
                response_data = response.json()
                self.log(
                    f"📥 Sessions list ({response_data['total_count']} total)",
                    {
                    ...
                    for line in response.iter_lines():
                        if line.startswith("data: "):
                            ...
                            try:
                                ...
                                # Log the full event
                                self.log(
                                    f"📨 SSE Event #{event_count} (type: {event_type}, event_data: {event_data})"
                                )
    ...
    def test_chat_history(self):
        ...
        try:
            response = httpx.get(
                f"{self.base_url}/api/v1/app/chat/{self.session_id}/history", headers=headers
            )
            ...
            if response.status_code == 200:
                ...
                self.log(
                    f"📥 Chat history ({len(history_data['messages'])} messages)",
                    {
