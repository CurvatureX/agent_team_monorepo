...
# 加载.env文件
env_path = "/Users/bytedance/personal/curvaturex/agent_team_monorepo/apps/backend/workflow_engine/.env"
    ...
    try:
        ...
        with get_db_session() as db:
            ...
            for row in result:
                ...
                if param_schema:
                    try:
                        schema = json.loads(param_schema) if isinstance(param_schema, str) else param_schema
                ...
                if default_params:
                    try:
                        defaults = json.loads(default_params) if isinstance(default_params, str) else default_params
