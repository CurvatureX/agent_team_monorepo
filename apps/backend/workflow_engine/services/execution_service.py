...
class Execution:
    """Minimal Execution model for compatibility"""
    ...
    def __init__(self, **kwargs):
        ...
        self.execution_metadata = kwargs.get("execution_metadata", {})
...
class ExecuteWorkflowRequest:
    """Minimal ExecuteWorkflowRequest for compatibility"""
...
class ExecuteSingleNodeRequest:
    """Minimal ExecuteSingleNodeRequest for compatibility"""
...
class SingleNodeExecutionResponse:
    """Minimal SingleNodeExecutionResponse for compatibility"""
...
class ExecutionService:
    ...
    def __init__(self, access_token: Optional[str] = None):
        ...
        # Initialize execution engine with database connection
        from database import Database

        self.database = Database()
        self.execution_engine = WorkflowExecutor(self.database)
        ...
        try:
            ...
            # Import and use full-featured business logger
            from utils.business_logger import create_business_logger

            self.log_service = create_business_logger()
            self.logger.info("✅ Business log service initialized with full utilities")
        except Exception as e:
            self.logger.error(f"❌ Failed to initialize log service: {e}")
    ...
    async def execute_single_node(
        ...
        try:
            ...
            workflow_definition = {
                ...
                "nodes": [
                    node.model_dump() if hasattr(node, "model_dump") else node.__dict__
                    for node in workflow.nodes
                ],
    ...
    async def _execute_workflow_async(self, execution_id: str, request: ExecuteWorkflowRequest):
        ...
        try:
            ...
            # Add business log: workflow started (simplified for migration)
            if self.log_service:
                try:
                    # Simplified logging approach for new structure
                    self.logger.info(f"🔍 Workflow started for execution {execution_id}")
                except Exception as log_error:
                    self.logger.error(f"❌ Failed to write workflow_started log: {log_error}")
