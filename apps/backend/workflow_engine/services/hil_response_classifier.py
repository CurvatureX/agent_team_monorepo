...
"""
    ...
    async def _call_gemini_api(self, prompt: str) -> Dict[str, Any]:
        ...
        Uses google-generativeai if available and API key configured; otherwise falls back
        to a safe heuristic with conservative scoring.
        """
        # Try real Gemini only if dependencies and keys are present
        api_key = __import__("os").getenv("GEMINI_API_KEY") or __import__("os").getenv(
            "GOOGLE_API_KEY"
        )
        try:
            if api_key:
                try:
                    ...
                    # Expect JSON in response per prompt instruction
                    ...
                    self.logger.warning(f"Gemini call failed, falling back to heuristic: {e}")
            else:
                self.logger.debug("Gemini API key not configured; using heuristic classifier")
        except Exception:
            # Environment import errors etc.
            self.logger.debug("Gemini environment not available; using heuristic classifier")

        # Heuristic fallback
        text_lc = prompt.lower()
        if any(tok in text_lc for tok in ["approv", "reject", "approved", "rejected"]):
            return {
                "relevance_score": 0.85,
                "reasoning": "Contains approval/rejection language relevant to interaction",
                "classification": "relevant",
            }
        if any(tok in text_lc for tok in ["yes", "no", "confirm", "deny"]):
            return {
                "relevance_score": 0.75,
                "reasoning": "Contains yes/no semantics potentially relevant to interaction",
                "classification": "relevant",
            }
        return {
            "relevance_score": 0.25,
            "reasoning": "No clear relevance indicators found in response; filtered out",
