...
class OAuth2ServiceLite:
    ...
    def __init__(self):
        """初始化OAuth2服务（基于 Supabase Python SDK）"""
    ...
    async def store_user_credentials(
        ...
        try:
            ...
            if existing.data:
                _id = existing.data[0]["id"]
                self.supabase.table("oauth_tokens").update(record).eq("id", _id).execute()
            else:
                self.supabase.table("oauth_tokens").insert(record).execute()
