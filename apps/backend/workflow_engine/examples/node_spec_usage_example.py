...
class ImprovedAIAgentNodeExecutor(BaseNodeExecutor):
    ...
    def _get_node_spec(self):
        ...
        if self.subtype and node_spec_registry:
            return node_spec_registry.get_spec(NodeType.AI_AGENT.value, self.subtype)
        return None
    ...
    def validate(self, node: Any) -> List[str]:
        ...
        # 额外的业务逻辑验证
        if node.subtype == AIAgentSubtype.OPENAI_NODE.value:
            # 检查API密钥是否配置
            import os
    ...
    def execute(self, context: NodeExecutionContext):
        ...
        if spec:
            ...
            temperature = context.get_parameter(
                "temperature", spec.get_parameter("temperature").default_value
            )
            max_tokens = context.get_parameter(
                "max_tokens", spec.get_parameter("max_tokens").default_value
            )
...
def create_ai_workflow_with_specs():
    ...
    openai_spec = node_spec_registry.get_spec(
        NodeType.AI_AGENT.value, AIAgentSubtype.OPENAI_NODE.value
    )
    http_spec = node_spec_registry.get_spec(NodeType.ACTION.value, ActionSubtype.HTTP_REQUEST.value)
    ...
    openai_node = {
        ...
        "subtype": AIAgentSubtype.OPENAI_NODE.value,
        "parameters": {},
    }
    ...
    workflow = {
        ...
        "connections": {
            "connections": {
                "DataAnalyzer": {
                    "connection_types": {
                        "main": {"connections": [{"node": "SendResults", "type": "MAIN"}]}
                    }
                }
            }
        },
...
class ParameterValidator:
    ...
    def validate_and_convert_parameters(
        node_type: str, subtype: str, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """验证并转换参数到正确的类型"""
...
async def validate_node_request(node_data: Dict[str, Any]):
    ...
    if errors:
        raise HTTPException(
            status_code=400, detail={"error": "Invalid node configuration", "details": errors}
        )
    ...
    def validate_workflow(workflow: Dict[str, Any]) -> List[str]:
        ...
        for source_name, conn_data in connections.items():
            ...
            source_spec = node_spec_registry.get_spec(
                source_node.get("type"), source_node.get("subtype")
            )
            ...
            if source_spec:
                ...
                for conn_type, conns in conn_data.get("connection_types", {}).items():
                    output_port = next(
                        (p for p in source_spec.output_ports if p.name == conn_type), None
                    )
                    if not output_port:
...
if __name__ == "__main__":
    ...
    # 3. 生成文档
    print("\n3. 自动生成节点文档:")
    doc = generate_node_documentation(NodeType.AI_AGENT.value, AIAgentSubtype.CLAUDE_NODE.value)
    print(doc)
