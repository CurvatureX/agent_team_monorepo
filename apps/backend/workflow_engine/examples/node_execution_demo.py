...
def create_mock_node(node_id: str, node_type: str, subtype: str, parameters: dict = None, credentials: dict = None):
...
def demo_trigger_node():
    ...
    executor = factory.get_executor("TRIGGER_NODE")
    
    # Test different trigger types
    trigger_types = [
        ("MANUAL", {}, {"input": "Manual trigger test"}),
        ("WEBHOOK", {"webhook_url": "https://api.example.com/webhook", "method": "POST"}, {"webhook_payload": {"data": "test"}}),
        ("CRON", {"cron_expression": "0 9 * * *"}, {}),
        ("CHAT", {"platform": "slack", "channel": "#general"}, {"message": {"text": "Hello from workflow!"}}),
        ("EMAIL", {"email_address": "test@example.com", "subject_filter": "workflow"}, {"email": {"subject": "Test Email"}}),
        ("FORM", {"form_fields": ["name", "email"]}, {"form_submission": {"name": "John", "email": "john@example.com"}}),
        ("CALENDAR", {"calendar_id": "primary", "event_type": "meeting"}, {"calendar_event": {"title": "Team Meeting"}})
    ]
    
    for subtype, parameters, input_data in trigger_types:
        print(f"\n--- Testing {subtype} Trigger ---")
        
        node = create_mock_node("trigger-1", "TRIGGER_NODE", subtype, parameters)
...
def demo_ai_agent_node():
    ...
    factory = get_node_executor_factory()
    executor = factory.get_executor("AI_AGENT_NODE")
    ...
    agent_types = [
        ("ROUTER_AGENT", {
            ...
            "model_name": "gpt-4",
            "routing_rules": [
                {"name": "calendar", "keywords": ["meeting", "schedule"], "route": "calendar_flow"},
    ...
    for subtype, parameters, input_data in agent_types:
        ...
        node = create_mock_node("ai-agent-1", "AI_AGENT_NODE", subtype, parameters)
        context = NodeExecutionContext(
            node=node,
            workflow_id="demo-workflow",
            execution_id="demo-exec-1",
...
def demo_external_action_node():
    ...
    factory = get_node_executor_factory()
    executor = factory.get_executor("EXTERNAL_ACTION_NODE")
    ...
    for subtype, parameters, input_data in action_types:
        ...
        node = create_mock_node("external-action-1", "EXTERNAL_ACTION_NODE", subtype, parameters)
        context = NodeExecutionContext(
            node=node,
            workflow_id="demo-workflow",
            execution_id="demo-exec-1",
...
def demo_action_node():
    ...
    factory = get_node_executor_factory()
    executor = factory.get_executor("ACTION_NODE")
    ...
    for subtype, parameters, input_data in action_types:
        ...
        node = create_mock_node("action-1", "ACTION_NODE", subtype, parameters)
        context = NodeExecutionContext(
            node=node,
            workflow_id="demo-workflow",
            execution_id="demo-exec-1",
...
def demo_flow_node():
    ...
    factory = get_node_executor_factory()
    executor = factory.get_executor("FLOW_NODE")
    ...
    for subtype, parameters, input_data in flow_types:
        ...
        node = create_mock_node("flow-1", "FLOW_NODE", subtype, parameters)
        context = NodeExecutionContext(
            node=node,
            workflow_id="demo-workflow",
            execution_id="demo-exec-1",
...
def demo_human_loop_node():
    ...
    factory = get_node_executor_factory()
    executor = factory.get_executor("HUMAN_IN_THE_LOOP_NODE")
    ...
    for subtype, parameters, input_data in interaction_types:
        ...
        node = create_mock_node("human-loop-1", "HUMAN_IN_THE_LOOP_NODE", subtype, parameters)
        context = NodeExecutionContext(
            node=node,
            workflow_id="demo-workflow",
            execution_id="demo-exec-1",
...
def demo_tool_node():
    ...
    factory = get_node_executor_factory()
    executor = factory.get_executor("TOOL_NODE")
    ...
    for subtype, parameters, input_data in tool_types:
        ...
        node = create_mock_node("tool-1", "TOOL_NODE", subtype, parameters)
        context = NodeExecutionContext(
            node=node,
            workflow_id="demo-workflow",
            execution_id="demo-exec-1",
...
def demo_memory_node():
    ...
    factory = get_node_executor_factory()
    executor = factory.get_executor("MEMORY_NODE")
    ...
    memory_types = [
        ...
        ("BUFFER", {
            "operation": "push",
            "buffer_name": "message_queue",
            "buffer_size": 100
        }, {"item": {"message": "Hello", "timestamp": "2024-01-15T10:00:00Z"}}),
        
        ("KNOWLEDGE_GRAPH", {
            "operation": "add_node"
        }, {"node_id": "person_1", "node_type": "person", "properties": {"name": "John", "age": 30}}),
        
        ("VECTOR_STORE", {
            "operation": "store",
            "collection_name": "documents"
        }, {"vector_id": "doc_1", "vector": [0.1, 0.2, 0.3], "metadata": {"text": "Sample document"}}),
        
        ("DOCUMENT_STORAGE", {
            "operation": "store",
            "document_id": "doc_123"
        }, {"content": "This is a sample document", "document_type": "text", "metadata": {"author": "John"}}),
    ...
    for subtype, parameters, input_data in memory_types:
        ...
        node = create_mock_node("memory-1", "MEMORY_NODE", subtype, parameters)
        context = NodeExecutionContext(
            node=node,
            workflow_id="demo-workflow",
            execution_id="demo-exec-1",
