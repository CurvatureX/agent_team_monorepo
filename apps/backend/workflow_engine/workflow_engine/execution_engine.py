...
class WorkflowExecutionEngine:
    ...
    def _initialize_enhanced_execution_state(
        ...
        return {
            ...
            "execution_path": {"steps": [], "node_execution_counts": {}},
            "node_inputs": {},
            "execution_context": {
                "workflow_variables": {},
                "execution_start_time": int(time.time()),
                "trigger_data": initial_data or {},
                "metadata": {},
            },
            "performance_metrics": {
            ...
            "data_flow": {"data_transfers": [], "data_sources": {}},
        }
    ...
    def _prepare_node_input_data_with_tracking(
        ...
        # Handle both simple connections list and complex ConnectionsMap
        if isinstance(connections, list):
            # Simple connections format
            return self._prepare_simple_connections_data(
                node_id, connections, node_results, initial_data
            )
        else:
            # Complex ConnectionsMap format
            return self._prepare_complex_connections_data(
                node_id, connections, node_results, initial_data, execution_state
            )

    def _prepare_simple_connections_data(
        self, node_id: str, connections: List[Dict], node_results: Dict, initial_data: Dict
    ) -> Dict[str, Any]:
        """Handle simple connections format."""
        ...
        for conn in connections:
            if conn.get("target") == node_id:
                ...
                if source_id in node_results:
                    ...
                    if source_result.get("status") == "SUCCESS" and "output_data" in source_result:
                        input_data.update(source_result["output_data"])

        return input_data

    def _prepare_complex_connections_data(
        self,
        node_id: str,
        connections: Dict,
        node_results: Dict,
        ...
        """Handle complex ConnectionsMap format with enhanced tracking."""
    ...
    def _calculate_execution_order(self, workflow_definition: Dict[str, Any]) -> List[str]:
        ...
        if isinstance(connections, list):
            return self._calculate_simple_execution_order(nodes, connections)
        else:
            return self._calculate_complex_execution_order(nodes, connections)

    def _calculate_simple_execution_order(
        self, nodes: List[Dict], connections: List[Dict]
    ) -> List[str]:
        """Calculate execution order for simple connections format."""
        ...
        while remaining:
            ...
            if not ready:
                return [node["id"] for node in nodes]  # Fallback
    ...
    def _calculate_complex_execution_order(
        self, nodes: List[Dict], connections: Dict[str, Any]
    ) -> List[str]:
        """Calculate execution order for complex ConnectionsMap format."""
