...
class GoogleCalendarAdapter(APIAdapter):
    ...
    async def _update_event(
        ...
        for field in updateable_fields:
            if field in parameters:
                if field in ["start", "end"]:
                    ...
                    update_data[field] = [{"email": email} for email in parameters[field]]
                else:
        ...
        response = await self.make_http_request("PUT", url, headers=headers, json_data=update_data)
    ...
    async def _delete_event(
        ...
        if response.status_code == 204:
            ...
            # 事件已被删除
            return {"success": True, "message": "Event was already deleted", "event_id": event_id}
        else:
            self._handle_http_error(response)
    ...
    async def _create_calendar(
        ...
        return {"success": True, "calendar": calendar, "calendar_id": calendar.get("id")}
    ...
    async def _search_events(
        ...
        # 使用list_events的搜索功能
        search_params = {"q": parameters["q"], "single_events": True, "order_by": "startTime"}
    ...
    async def _quick_add(
        ...
        return {"success": True, "event": event, "event_id": event.get("id"), "parsed_text": text}
    ...
    async def _watch_events(
        ...
        response = await self.make_http_request("POST", url, headers=headers, json_data=watch_data)
    ...
    async def _stop_watching(
        ...
        stop_data = {"id": channel_id, "resourceId": resource_id}

        url = f"{self.BASE_URL}/channels/stop"
        headers = self._prepare_headers(credentials)

        response = await self.make_http_request("POST", url, headers=headers, json_data=stop_data)
    ...
    def _format_datetime(self, dt_input) -> str:
        ...
        if dt_input is None:
            ...
            raise ValidationError(f"Invalid datetime format: {type(dt_input)} (value: {dt_input})")
