...
class EmailAdapter(APIAdapter):
    ...
    def _add_attachment(self, msg: MIMEMultipart, attachment: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.warning(f"Failed to add attachment {attachment.get('filename', 'unknown')}: {e}")
    ...
    async def connection_test(self, credentials: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await loop.run_in_executor(
                ...
                smtp_server, port, username, password, use_tls
            )
    ...
    def _test_smtp_connection(self, smtp_server: str, port: int, username: str, password: str, use_tls: bool) -> Dict[str, Any]:
        """同步测试SMTP连接"""
        try:
            if port == 465:
                # SSL连接
