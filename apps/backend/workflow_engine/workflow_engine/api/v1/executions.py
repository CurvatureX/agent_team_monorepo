...
async def execute_workflow(request: ExecuteWorkflowRequest, service: ExecutionService = Depends(get_execution_service)):
    try:
...
async def get_execution_status(execution_id: str, service: ExecutionService = Depends(get_execution_service)):
    try:
...
async def cancel_execution(execution_id: str, service: ExecutionService = Depends(get_execution_service)):
    try:
...
async def get_execution_history(workflow_id: str, limit: int = 50, service: ExecutionService = Depends(get_execution_service)):
    try:
