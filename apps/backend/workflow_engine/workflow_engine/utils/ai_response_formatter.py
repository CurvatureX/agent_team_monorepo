...
class AIResponseFormatter:
    ...
    def format_response(
        ...
        extracted_fields = AIResponseFormatter._extract_common_fields(
    ...
    def _extract_common_fields(
        ...
        if structured_data:
            ...
            if "tasks" in structured_data:
                ...
                extracted["first_task"] = structured_data["tasks"][0] if structured_data["tasks"] else None
        ...
        # Extract from text if no structured data
            ...
            extracted["has_urgent"] = any(word in response_lower for word in ["urgent", "asap", "immediately"])
            extracted["has_action"] = any(word in response_lower for word in ["action", "task", "todo", "must"])
