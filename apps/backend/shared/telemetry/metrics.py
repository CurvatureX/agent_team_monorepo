...
def get_metrics(service_name: str) -> ServiceMetrics:
    ...
    request_count = meter.create_counter(
        name="request_count", description="Total number of HTTP requests", unit="1"
    )

    request_duration = meter.create_histogram(
        name="request_duration_seconds", description="HTTP request duration in seconds", unit="s"
    )

    request_errors = meter.create_counter(
        name="request_errors_total", description="Total number of HTTP request errors", unit="1"
    )

    active_requests = meter.create_up_down_counter(
        name="active_requests", description="Number of active HTTP requests", unit="1"
    )

    # 业务指标
    api_key_usage = meter.create_counter(
        name="api_key_usage_total", description="API key usage by client", unit="1"
    )

    endpoint_usage = meter.create_counter(
        name="endpoint_usage_total", description="Endpoint usage frequency", unit="1"
    )

    user_activity = meter.create_counter(
        name="user_activity_total", description="User activity events", unit="1"
    )

    # AI 专项指标
    ai_requests = meter.create_counter(
        name="ai_requests_total", description="Total AI model requests", unit="1"
    )
    ...
    ai_cost = meter.create_counter(
        name="ai_cost_total", description="Total AI cost in USD", unit="1"
    )

    # 工作流指标
    workflow_success_rate = meter.create_histogram(
        name="workflow_success_rate", description="Workflow execution success rate", unit="1"
    )
...
def get_standard_labels(
    service_name: str, endpoint: str = "", method: str = "", status_code: str = "", **kwargs
) -> Dict[str, Any]:
    """
    获取标准化标签
