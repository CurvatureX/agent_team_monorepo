...
class MetricsMiddleware(BaseHTTPMiddleware):
    ...
    def _record_business_metrics(self, request: Request, response: Response, tracking_id: str) -> None:
        """记录业务相关指标"""
        try:
            # API 使用情况
            if hasattr(request.state, 'api_key_id'):
                self.metrics.api_key_usage.add(1, {
            ...
            # 端点使用统计
            self.metrics.endpoint_usage.add(1, {
                'service_name': self.service_name,
                'endpoint': self._normalize_endpoint(request.url.path),
                'api_version': self._extract_api_version(request.url.path),
            ...
            # 用户活动
            if hasattr(request.state, 'user_id'):
                self.metrics.user_activity.add(1, {
