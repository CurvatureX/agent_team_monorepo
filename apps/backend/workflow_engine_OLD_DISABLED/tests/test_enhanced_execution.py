        ...
        "nodes": [
            {
                ...
                "parameters": {"message": "Hello from enhanced execution engine"},
            },
            {
                "id": "action-1",
                ...
                "parameters": {"operation": "uppercase", "field": "message"},
            },
            {
                "id": "memory-1",
                "name": "Store Result",
                ...
                "parameters": {"key": "processed_message"},
            },
        ],
        "connections": {
            "Manual Trigger": {
            ...
            "Data Processing": {
                "connection_types": {
                    "main": {"connections": [{"node": "Store Result", "type": "MAIN", "index": 0}]}
                }
            },
        },
        "static_data": {"environment": "test", "version": "1.0.0"},
    }
...
def test_enhanced_execution_engine():
    ...
    credentials = {"api_key": "test_key_123", "user_id": "test_user"}
...
def print_execution_report(report):
    ...
    for transfer in data_flow["data_transfers"]:
        print(
            f"  {transfer['node_name']}: {transfer['input_data_size']} → {transfer['output_data_size']} bytes"
        )
    print()
