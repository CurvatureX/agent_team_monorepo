...
class ExecutionService:
    ...
    async def execute_workflow(self, request: ExecuteWorkflowRequest) -> str:
        ...
        try:
            ...
            task = loop.create_task(
                self._handle_workflow_execution(execution_id=execution_id, request=request)
            )
    ...
    async def _prepare_and_execute_workflow(
        self, execution_id: str, workflow_id: str, user_id: str, request: ExecuteWorkflowRequest
    ):
        """准备并执行workflow的后台任务"""
        try:
            ...
            if not workflow:
                self.logger.error(f"❌ Workflow not found: {workflow_id} for user: {user_id}")
                # Update execution status to ERROR
                self._update_execution_status(
                    execution_id, ExecutionStatus.ERROR.value, "Workflow not found"
                )
                return
            ...
            self.logger.info(f"📋 Workflow definition nodes: {len(workflow_dict.get('nodes', []))}")
            ...
            if start_from_node:
                ...
                workflow_dict = self._modify_workflow_for_start_node(
                    workflow_dict, start_from_node, skip_trigger_validation
                )
            ...
            if start_from_node and hasattr(request, "inputs") and request.inputs:
                ...
                self.logger.info(
                    f"🎯 Using custom inputs for start_from_node: {list(initial_data.keys())}"
                )
            else:
            ...
            self.logger.error(
                f"Background workflow execution failed: {execution_id} - {exec_error}"
            )
            self._update_execution_status(
                execution_id, ExecutionStatus.ERROR.value, str(exec_error)
            )
    ...
    async def _execute_workflow_background(
        ...
        try:
            ...
            if db_execution:
                ...
                self.logger.error(
                    f"❌ Execution record not found for background update: {execution_id}"
                )
