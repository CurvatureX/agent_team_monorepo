            ...
            validation_result = self.validator.validate_workflow_structure(
                {
                    ...
                    if request.settings
                    else {},  # Fix Pydantic serialization
                },
            ...
            if update_data.settings is not None:
                ...
                if current_workflow and current_workflow.get("workflow_data"):
                    ...
                    workflow_data["settings"] = (
                        update_data.settings.dict() if update_data.settings else {}
                    )
                ...
                workflow_data.update(
                    {
                        "nodes": [node.dict() for node in temp_nodes],
                        "connections": connections_data,
                        "settings": update_data.settings.dict()
                        if update_data.settings
                        else workflow_data.get("settings", {}),
                    }
                )
