...
class ExecutionService:
    ...
    def __init__(self, access_token: Optional[str] = None):
        ...
        # Initialize unified log service for structured business logging
        try:
            self.logger.info("🔍 Initializing unified log service...")
            from workflow_engine.services.unified_log_service import get_unified_log_service

            self.log_service = get_unified_log_service()
            self.logger.info("✅ Unified log service initialized successfully")
        except Exception as e:
            self.logger.error(f"❌ Failed to initialize unified log service: {e}")
    ...
    async def _create_execution_record(self, execution_id: str, request: ExecuteWorkflowRequest):
        """Create a minimal execution record for tracking."""
        try:
            from datetime import datetime

            from shared.models.execution import ExecutionStatus
            ...
            self.logger.info(f"✅ Created execution record for {execution_id}")

        except Exception as e:
            self.logger.error(f"Failed to create execution record for {execution_id}: {e}")
            raise
    ...
    async def _execute_workflow_async(self, execution_id: str, request: ExecuteWorkflowRequest):
        ...
        try:
            ...
            # Add business log: workflow started
            if self.log_service:
                try:
                    self.logger.info(
                        f"🔍 Attempting to write workflow_started log for execution {execution_id}"
                    )
                    await self.log_service.add_business_log(
                        execution_id=execution_id,
                        event_type="workflow_started",
                        technical_message=f"Starting workflow execution {request.workflow_id}",
                        user_friendly_message=f"Workflow execution started",
                        level="INFO",
                        is_milestone=True,
                        display_priority=8,
                    )
                    self.logger.info(
                        f"✅ Successfully wrote workflow_started log for execution {execution_id}"
                    )
                except Exception as log_error:
                    self.logger.error(
                        f"❌ Failed to write workflow_started log for execution {execution_id}: {log_error}"
                    )
                    import traceback
                ...
                self.logger.warning(
                    f"⚠️ Log service not available, skipping workflow_started log for execution {execution_id}"
                )
            ...
            if execution_status == "completed":
                ...
                # Add business log: workflow completed
                if self.log_service:
                    node_results = execution_result.get("node_results", {})
                    total_nodes = len(node_results)
                    successful_nodes = sum(
                        1 for result in node_results.values() if result.get("status") == "SUCCESS"
                    )
                    ...
                    try:
                        await self.log_service.add_business_log(
                            ...
                            technical_message=f"Workflow execution completed successfully with {successful_nodes}/{total_nodes} nodes successful",
                            user_friendly_message=f"Workflow completed successfully. Executed {successful_nodes} out of {total_nodes} steps.",
                            level="INFO",
                            is_milestone=True,
                            display_priority=9,
                            data={
                                "total_steps": total_nodes,
                                "successful_steps": successful_nodes,
                                "status": "completed",
                            },
                        )
                    except Exception as log_error:
                        self.logger.error(f"❌ Failed to write workflow_completed log: {log_error}")
                else:
                    self.logger.warning(
                        f"⚠️ Log service not available, skipping workflow_completed log"
                    )
                ...
                # Add business log: workflow error
                if self.log_service:
                    node_results = execution_result.get("node_results", {})
                    total_nodes = len(node_results)
                    failed_nodes = sum(
                        1 for result in node_results.values() if result.get("status") == "ERROR"
                    )
                    ...
                    try:
                        await self.log_service.add_business_log(
                            ...
                            technical_message=f"Workflow execution failed: {error_msg}",
                            user_friendly_message=f"Workflow execution failed. {failed_nodes} out of {total_nodes} steps encountered errors.",
                            level="ERROR",
                            is_milestone=True,
                            display_priority=10,
                            data={
                        ...
                        self.logger.error(f"❌ Failed to write workflow_error log: {log_error}")
                else:
                    self.logger.warning(
                        f"⚠️ Log service not available, skipping workflow_error log"
                    )

        except Exception as e:
            self.logger.error(f"❌ Error in async workflow execution {execution_id}: {e}")

            # Add business log: execution exception
            if self.log_service:
                try:
                    await self.log_service.add_business_log(
                        execution_id=execution_id,
                        event_type="execution_error",
                        technical_message=f"Execution exception: {str(e)}",
                        user_friendly_message=f"Workflow execution encountered an unexpected error: {str(e)[:100]}{'...' if len(str(e)) > 100 else ''}",
                        level="ERROR",
                        is_milestone=True,
                        display_priority=10,
                        data={
                    ...
                    self.logger.error(f"❌ Failed to add error log: {log_error}")
            else:
                self.logger.warning(f"⚠️ Log service not available, skipping execution_error log")
