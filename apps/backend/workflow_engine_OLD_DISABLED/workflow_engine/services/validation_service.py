...
class ValidationService:
    ...
    def validate_workflow(
        self, request: workflow_service_pb2.ValidateWorkflowRequest, context: grpc.ServicerContext
        ...
        try:
                ...
                try:
                    if self.node_factory:
                        ...
                        if executor:
                            ...
                            errors.append(
                                f"Node {node.id}: No executor found for type {node.type}, subtype {node.subtype}"
                            )
                    else:
                        warnings.append(
                            f"Node {node.id}: Node factory not available, skipping validation"
                        )
            ...
            if request.workflow.connections:
                connection_errors = self._validate_connections_map(
                    request.workflow.connections, node_names
                )
    ...
    def _validate_connections_map(
        self, connections_map: workflow_pb2.ConnectionsMap, node_names: Set[str]
    ) -> List[str]:
    ...
    def test_node(
        self, request: workflow_service_pb2.TestNodeRequest, context: grpc.ServicerContext
        ...
        try:
            ...
            try:
                ...
                executor = self.node_factory.create_executor(
                    request.node.type, request.node.subtype
                )
                if not executor:
                    return workflow_service_pb2.TestNodeResponse(
                        success=False,
