...
{{ $node["ai_planning_facilitator"].json.extracted.task_count }} tasks created
    ...
    workflow_data = {"node_outputs": {"ai_planning_facilitator": planning_node_output}}
...
def example_message_classification_workflow():
    ...
    workflow_data = {"node_outputs": {"ai_message_classification": classification_output}}

    # Conditional routing based on classification
    proxy = WorkflowDataProxy(
        {"node_results": {"ai_message_classification": {"output_data": classification_output}}}
    )

    priority = proxy.get_node_parameter("ai_message_classification", "priority")
    category = proxy.resolve_expression_for_node(
        "router", '$node["ai_message_classification"].json.category'
    )
...
- Hire 2 new engineers
    ...
    print(
        f"  Bullet points found: {len(report_output['json']['extracted'].get('bullet_points', []))}"
    )
