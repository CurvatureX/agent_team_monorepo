...
# 🚨 MODULE LEVEL DEBUG - This should ALWAYS appear in logs
print("🚨 MODULE LEVEL DEBUG: executions.py is being imported!")
import logging

module_logger = logging.getLogger(__name__)
module_logger.error("🚨 MODULE LEVEL DEBUG: executions.py imported with logging!")
...
async def execute_workflow(
    ...
    logger.error(f"🔥 CRITICAL DEBUG: ENDPOINT HIT - execute_workflow called")
    logger.error(f"🔥 CRITICAL DEBUG: request type = {type(request)}")
    logger.error(f"🔥 CRITICAL DEBUG: request.__dict__ = {request.__dict__}")
    logger.error(
        f"🔥 CRITICAL DEBUG: async_execution = {getattr(request, 'async_execution', 'MISSING FIELD')}"
    )
    logger.error(f"🔥 CRITICAL DEBUG: async_execution value = {request.async_execution}")
    try:
        # 获取 trace_id
        ...
        # URGENT DEBUG: Print request to see if async_execution is received
        print(f"🔍 URGENT DEBUG: request.__dict__ = {request.__dict__}")
        print(f"🔍 URGENT DEBUG: async_execution = {getattr(request, 'async_execution', 'MISSING')}")

        # Check if async execution is requested
        if getattr(request, "async_execution", False):
            # SIMPLE ASYNC: Start workflow in background and return immediately
            import asyncio

            # Start workflow execution in background task
            asyncio.create_task(service.execute_workflow(request))

            # Return immediately with a dummy execution ID
            import uuid
            ...
            # For sync execution: wait for completion (original behavior)
