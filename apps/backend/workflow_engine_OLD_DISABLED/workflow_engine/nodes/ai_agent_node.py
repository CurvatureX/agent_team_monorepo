...
class AIAgentNodeExecutor(BaseNodeExecutor):
    ...
    def __init__(self, subtype: Optional[str] = None):
        ...
        if PROVIDERS_AVAILABLE:
            ...
            self.logger.warning(
                "[AIAgent Node]: 🤖 AI AGENT: AI providers not available, will use legacy API calls"
            )
    ...
    def _execute_gemini_agent(
        ...
        try:
            ...
            system_prompt = self._enhance_system_prompt_with_summary(
            ...
            if response.raw_response:
                self.logger.info(
                    f"[AIAgent Node]: 🔍 Raw AI Response (Gemini): {response.raw_response}"
                )
            else:
    ...
    def _execute_openai_agent(
        ...
        try:
            ...
            if response.raw_response:
                self.logger.info(
                    f"[AIAgent Node]: 🔍 Raw AI Response (OpenAI): {response.raw_response}"
                )
            else:
    ...
    def _execute_claude_agent(
        ...
        try:
            ...
            if response.raw_response:
                self.logger.info(
                    f"[AIAgent Node]: 🔍 Raw AI Response (Claude): {response.raw_response}"
                )
            else:
