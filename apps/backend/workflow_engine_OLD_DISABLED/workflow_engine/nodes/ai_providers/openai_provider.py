...
class OpenAIProvider(AIProviderInterface):
        ...
        try:
            ...
            if not self.api_key:
                return self.create_error_response(
                    ErrorType.AUTH_ERROR, "OpenAI API key not found in environment or parameters"
                )
            ...
            try:
                ...
                return self.create_error_response(
                    ErrorType.UNKNOWN, "OpenAI library not installed. Run: pip install openai"
                )
            ...
            if hasattr(response, "model_dump_json"):
                ...
                self.logger.info(
                    f"OpenAI raw response type: {type(response)}, choices: {len(response.choices) if response.choices else 0}"
                )
                if response.choices and len(response.choices) > 0:
                    self.logger.info(
                        f"First choice: message={response.choices[0].message}, finish_reason={response.choices[0].finish_reason}"
                    )
    ...
    def validate_model(self, model: str) -> Tuple[bool, str]:
        ...
        return False, (
            f"Invalid OpenAI model '{model}'. " f"Supported models: {', '.join(supported_models)}"
        )
