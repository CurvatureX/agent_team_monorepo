...
class ClaudeProvider(AIProviderInterface):
        ...
        try:
            ...
            if not self.api_key:
                return self.create_error_response(
                    ErrorType.AUTH_ERROR, "Anthropic API key not found in environment or parameters"
                )
            ...
            try:
                ...
                return self.create_error_response(
                    ErrorType.UNKNOWN, "Anthropic library not installed. Run: pip install anthropic"
                )
            ...
            if not response.content:
                return self.create_error_response(
                    ErrorType.RESPONSE_ERROR, "Empty response from Claude API", response
                )
            ...
            if not content:
                return self.create_error_response(
                    ErrorType.RESPONSE_ERROR, "No text content in Claude response", response
                )
    ...
    def validate_model(self, model: str) -> Tuple[bool, str]:
        ...
        return False, (
            f"Invalid Claude model '{model}'. " f"Supported models: {', '.join(supported_models)}"
        )
