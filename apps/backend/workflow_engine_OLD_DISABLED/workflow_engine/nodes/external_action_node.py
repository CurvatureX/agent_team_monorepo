...
class ExternalActionNodeExecutor(BaseNodeExecutor):
    ...
    def _prepare_slack_operation(self, context: NodeExecutionContext) -> tuple[str, Dict[str, Any]]:
                    ...
                    if isinstance(content_value, str) and content_value.strip().startswith("{"):
                        try:
                            ...
                            if isinstance(parsed_content, dict) and any(
                                key in parsed_content for key in ["message", "channel", "bot_token"]
                            ):
                                ...
                                if "channel" in parsed_content:
                                    self.logger.info(
                                        f"📢 AI suggested channel: {parsed_content['channel']}, but keeping configured channel: {channel}"
                                    )
                            else:
        ...
        self.logger.info(
            f"✅ Returning Slack operation: action='{action}', parameters keys={list(parameters.keys())}"
        )
        return action, parameters
    ...
    async def _get_user_credentials(self, user_id: str, provider: str) -> Optional[Dict[str, str]]:
        ...
        """
        # TEMPORARY FIX: Skip credential lookup to prevent blocking workflow execution
        # This allows workflows to execute without external API credentials for testing
        self.logger.warning(
            f"⚠️ TEMPORARY FIX: Skipping credential lookup for {provider} to prevent execution timeout. "
            f"External API calls for {provider} will use fallback/mock responses."
        )
        return None

        # TODO: Implement proper async credential retrieval
        # The original blocking code has been temporarily disabled to fix execution timeouts
        # Original issue: sync database operations were blocking async workflow execution
                ...
                if isinstance(content_value, str) and content_value.strip().startswith("{"):
                    try:
                        ...
                        if isinstance(parsed_content, dict) and any(
                            ...
                            self.logger.info(
                                f"✅ Extracted message from JSON content: {message_text}"
                            )
